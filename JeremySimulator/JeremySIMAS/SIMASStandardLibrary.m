...
SIMASFUNC(setVar) {
    ...
    if ([[[args objectAtIndex:0] lowercaseString] isEqualToString:@"in"]) {
        [targetVar setData:[SIMASString stringWithString:formatEscapes([[JeremyUIController theController] getUserInput])]];
        return;
    }
...
SIMASFUNC(consolePrint) {
    ...
    [[JeremyUIController theController] log:[[var data] toString]];
}

SIMASFUNC(consolePrintc) {
    [[JeremyUIController theController] log:[args objectAtIndex:0]];
}

SIMASFUNC(consolePrints) {
    [[JeremyUIController theController] log:@" "];
}
